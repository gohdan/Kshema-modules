...
function auto_models_prices_view()
{
	debug ("*** auto_models_prices_view ***");
	global $config;
    global $user;
    $content = array(
    	'content' => '',
	...
	debug ("*** end: auto_models_prices_view ***");
    return $content;
}

function auto_models_prices_edit()
{
	debug ("*** auto_models_prices_edit ***");
	global $config;
    global $user;
    $content = array(
    	'content' => '',
	...
	if (1 == $user['id'])
	{
		...
		{
			$sql_query = "SELECT count(*) FROM ksh_auto_models_prices WHERE model='".mysql_real_escape_string($model_id)."'";
			$result = exec_query($sql_query);
			$if_record_exist = mysql_result($result, 0, 0);
			mysql_free_result($result);

			if ($if_record_exist > 0)
				$sql_query = "UPDATE ksh_auto_models_prices SET full_text='".mysql_real_escape_string($_POST['full_text'])."' WHERE model='".mysql_real_escape_string($model_id)."'";
			else
